...
def read_wallet(json_db, walletfile, print_wallet, print_wallet_transactions, transaction_filter, include_balance, vers= -1, FillPool=False):
        ...
        def item_callback(type, d):
                if type == "tx":
                        ...
                        crypted = True
                        compressed = d['public_key'][0] != '\04'
                        json_db['keys'].append({'addr' : public_key_to_bc_address(d['public_key']), 'sec' : 'Encrypted', 'hexsec' : d['encrypted_private_key'].encode('hex'), 'secret' : d['encrypted_private_key'].encode('hex'), 'pubkey':d['public_key'].encode('hex'), 'encrypted_private_key':d['encrypted_private_key'].encode('hex'), 'compressed':compressed})

                elif type == "mkey":
                        json_db['mkey'] = {
                                'salt': d['salt'].encode('hex_codec'),
                                'nDerivationIterations': d['nDerivationIterations'],
                                'nDerivationMethod': d['nDerivationMethod'],
                                'encrypted_key': d['encrypted_key'].encode('hex_codec')
                        }
...
if __name__ == '__main__':
        if len(sys.argv) != 2:
                sys.stderr.write("Usage: %s <wallet.dat file>\n" % sys.argv[0])
                sys.exit(-1)

        db = {}
        read_wallet(db, sys.argv[1], True, True, "", True, -1, False)
        
        if 'mkey' in db and 'keys' in db:
                # Output format for John the Ripper
                for key_data in db['keys']:
                        if 'encrypted_private_key' in key_data:
                                # Print hash in format expected by John the Ripper
                                print "%s:$bitcoin$%s$%s$%s$%s$%s$%s$%s$%s" % (
                                        sys.argv[1],
                                        len(key_data['encrypted_private_key']) / 2,
                                        key_data['encrypted_private_key'],
                                        len(db['mkey']['salt']) / 2,
                                        db['mkey']['salt'],
                                        db['mkey']['nDerivationIterations'],
                                        len(db['mkey']['encrypted_key']) / 2,
                                        db['mkey']['encrypted_key'],
                                        db['mkey']['nDerivationMethod']
                                )
                                break
        else:
                sys.stderr.write("No encrypted keys found in wallet\n")