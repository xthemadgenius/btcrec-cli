...
from opencl_brute.buffer_structs import buffer_structs
...
class opencl_interface:
    ...
    def compile(self, bufferStructsObj, library_file, footer_file=None, N=15, invMemoryDensity=2):
        ...
        if library_file:
            with open(os.path.join("opencl_brute","worker","generic",library_file), "r") as rf:
                src += rf.read()

        if footer_file:
            with open(os.path.join("opencl_brute","worker","generic",footer_file), "r") as rf:
                src += rf.read()
